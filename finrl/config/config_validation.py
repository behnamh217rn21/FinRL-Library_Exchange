...
def validate_config_schema(conf: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.critical(f"Invalid configuration. See config.json.example. Reason: {e}")
...
def _validate_trailing_stoploss(conf: Dict[str, Any]) -> None:
    ...
    if tsl_only_offset:
        if tsl_positive == 0.0:
            raise OperationalException('The config trailing_only_offset_is_reached needs '
                                       'trailing_stop_positive_offset to be more than 0 in your config.')
    if tsl_positive > 0 and 0 < tsl_offset <= tsl_positive:
        raise OperationalException('The config trailing_stop_positive_offset needs '
                                   'to be greater than trailing_stop_positive in your config.')

    # Fetch again without default
    if 'trailing_stop_positive' in conf and float(conf['trailing_stop_positive']) == 0.0:
        raise OperationalException('The config trailing_stop_positive needs to be different from 0 '
...
def _validate_edge(conf: Dict[str, Any]) -> None:
    ...
    if conf.get('pairlist', {}).get('method') == 'VolumePairList':
        raise OperationalException("Edge and VolumePairList are incompatible, "
                                   "Edge will override whatever pairs VolumePairlist selects.")
    if not conf.get('ask_strategy', {}).get('use_sell_signal', True):
        raise OperationalException("Edge requires `use_sell_signal` to be True, otherwise no sells will happen.")
...
def _validate_whitelist(conf: Dict[str, Any]) -> None:
    ...
    for pl in conf.get('pairlists', [{'method': 'StaticPairList'}]):
        if (pl.get('method') == 'StaticPairList' and not conf.get('exchange', {}).get('pair_whitelist')):
