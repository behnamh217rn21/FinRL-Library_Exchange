...
from finrl.exceptions import (DDosProtection, InsufficientFundsError, 
...
class Kraken(Exchange):
    ...
    _ft_has: Dict = {"stoploss_on_exchange": True,
    ...
    def get_balances(self) -> dict:
        ...
        try:
            ...
            balances.pop("info", None); balances.pop("free", None)
            balances.pop("total", None); balances.pop("used", None)
    ...
    def stoploss_adjust(self, stop_loss: float, order: Dict) -> bool:
        ...
        """
        return (order['type'] in ('stop-loss', 'stop-loss-limit') and stop_loss > float(order['price']))
    ...
    def stoploss(self, pair: str, amount: float, stop_price: float, order_types: Dict) -> Dict:
        ...
        if self._config['dry_run']:
            dry_order = self.dry_run_order(pair, ordertype, "sell", 
                                           amount, stop_price)
            return dry_order
        ...
        try:
            ...
            raise InsufficientFundsError(f'Insufficient funds to create {ordertype} sell order on market {pair}. '
                                         f'Tried to create stoploss with amount {amount} at stoploss {stop_price}. '
                                         f'Message: {e}') from e
        except ccxt.InvalidOrder as e:
            raise InvalidOrderException(f'Could not create {ordertype} sell order on market {pair}. '
                                        f'Tried to create stoploss with amount {amount} at stoploss {stop_price}. '
                                        f'Message: {e}') from e
        except ccxt.DDoSProtection as e:
            raise DDosProtection(e) from e
