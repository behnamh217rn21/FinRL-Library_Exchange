...
class TestStocktradingEnvCashpenalty(unittest.TestCase):
    def setUp(cls):
        ...
        cls.df = YahooDownloader(start_date="2009-01-01", end_date="2021-01-01", ticker_list=cls.ticker_list).fetch_data()
    ...
    def test_zero_step(self):
        ...
        init_amt = 1e6
        env = StockTradingEnvCashpenalty(df=self.df, initial_amount=init_amt, cache_indicator_data=False)
        _ = env.reset()
    ...
    def test_patient(self):
        ...
        hmax = aapl_first_close * 100
        env = StockTradingEnvCashpenalty(df=self.df, 
                                         initial_amount=init_amt, 
                                         hmax=hmax,
                                         cache_indicator_data=False,
                                         patient=True,
    ...
    def validate_caching(self):
        ...
        env_uncached = StockTradingEnvCashpenalty(df=self.df, initial_amount=init_amt, cache_indicator_data=False)
        env_cached = env = StockTradingEnvCashpenalty(df=self.df, initial_amount=init_amt, cache_indicator_data=True)
